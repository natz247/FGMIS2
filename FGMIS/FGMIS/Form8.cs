...
namespace FGMIS
{
    public partial class Form8 : Form
    {
        ...
        public Form8(string title, int activityId)
        {
            InitializeComponent();
            this.title = title;
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            if (fieldsValid())
            {
                string[] row = { dataGridView2.RowCount + "", textBox5.Text, numericUpDown3.Value.ToString(), comboBox7.Text, textBox8.Text, GetStatusOfRetention(), textBox1.Text };
                dataGridView2.Rows.Add(row);
                textBox5.Text = string.Empty;
            }
        }

        private string GetStatusOfRetention()
        {
            string statusOfRetention = "";

            if (radioButton1.Checked)
                statusOfRetention = "Enrolled";

            else if (radioButton2.Checked)
                statusOfRetention = "Promoted to next grade";

            else if (radioButton3.Checked)
                statusOfRetention = "Drop out";

            else
                statusOfRetention = "Repeat to same grade";

            return statusOfRetention;
        }
        ...
        private Boolean fieldsValid()
        {
            ...
            string gradelevel = (textBox8.Text).Trim();
            ...
            else if (gradelevel.CompareTo(string.Empty) == 0)
            {
                MessageBox.Show("Please enter a valid grade level!", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            else
            {
        ...
        private Boolean mainFieldsValid()
        {
            ...
            string schoolname = (textBox11.Text).Trim();
        ...
        private void SaveData(int saveOrUpdate)
        {
            ...
            if (mainFieldsValid())
            {
                ...
                activity.Schoolname = textBox11.Text;
                DateTime newTime = dateTimePicker1.Value;
                activity.Year = newTime.Year.ToString();
                activity.ActivityDate = dateTimePicker2.Value;
                ...
                if (dataGridView2.RowCount > 1)
                {
                    ...
                    {
                        Participant8 participant = new Participant8();
                        if (dataGridView2.Rows[i].Cells[1].Value != null)
                            participant.Name = dataGridView2.Rows[i].Cells[1].Value.ToString();
                        
                        if (dataGridView2.Rows[i].Cells[2].Value != null)
                            participant.Age = Convert.ToInt32(dataGridView2.Rows[i].Cells[2].Value.ToString());
                        ...
                        if (dataGridView2.Rows[i].Cells[4].Value != null)
                            participant.Gradelevel = dataGridView2.Rows[i].Cells[4].Value.ToString();
                        
                        if (dataGridView2.Rows[i].Cells[5].Value != null)
                            participant.Statusofretention = dataGridView2.Rows[i].Cells[5].Value.ToString();
                        
                        if (dataGridView2.Rows[i].Cells[6].Value != null)
                            participant.Remark = dataGridView2.Rows[i].Cells[6].Value.ToString();
